...
func _input(event):
	if(event.is_action_pressed("shoot") and canShoot):
		match(currentGun):
			...
			"Flamethrower": 
				if(GlobalGamePlayVariables.player_FlameBullet > 0):
					shootFire()
	...
	if(event.is_action_pressed("weapon1")): currentGun = "Basic"
	if(event.is_action_pressed("weapon2")): currentGun = "Shotgun"
...
func shootFire():
	canShoot = false
	cooldown = 0.4 - clamp((0.4 * GlobalGamePlayVariables.flamethrowerFirerateMultiplier),0.0,0.4)
	var location = arm.weaponSprite.global_position
	location.y += 3
	var Bdirection = (get_global_mouse_position() - arm.global_position).normalized()
...
func shootSlash():
	...
	location.y += 3
	var Bdirection = (get_global_mouse_position() - arm.global_position).normalized()
	PlayerBulletMaker.createKnifeSlash(location,int(get_global_mouse_position().x > global_position.x),arm)
	cooldownTimer.start(cooldown)
